...
describe("FreezeMode — ExposureRegistry + StableGuardCREReceiver", function () {
    ...
    async function driveToRecovery() {
        // Each report must have a strictly newer observedAt — encode per-iteration.
        for (let i = 0; i < STABILITY_WINDOW; i++) {
            await receiver.connect(forwarder).onReport(
                "0x",
                encodeReport([coinA.address], [0], 0)
            );
        }
    }
    ...
    // ── partial release / freeze-mode downgrade ───────────────────────────────

    describe("partial release — FULL_FREEZE downgrade to DEPOSIT_ONLY_FREEZE", function () {
        it("releasing the FULL_FREEZE hold leaves paused=false and depositsFrozen=true when a DEPOSIT_ONLY hold remains", async function () {
            const [, , , coinB] = await ethers.getSigners();
            const vaultAddr = await vault.getAddress();
            const symA = addrToBytes32(coinA.address);
            const symB = addrToBytes32(coinB.address);

            await registry.connect(admin).registerExposure(vaultAddr, symA);
            await registry.connect(admin).registerExposure(vaultAddr, symB);

            // Incident A opens under FULL_FREEZE (default) → vault.paused = true, hold A = FULL_FREEZE
            await receiver.connect(forwarder).onReport("0x", encodeReport([coinA.address], [2], 2));
            expect(await vault.paused()).to.equal(true);
            expect(await vault.depositsFrozen()).to.equal(false);

            // Admin switches vault to DEPOSIT_ONLY_FREEZE
            await registry.connect(admin).setFreezeMode(vaultAddr, FREEZE_MODE_DEPOSIT_ONLY_FREEZE);

            // Incident B opens under DEPOSIT_ONLY_FREEZE → vault.depositsFrozen = true, hold B = DEPOSIT_ONLY_FREEZE
            await receiver.connect(forwarder).onReport("0x", encodeReport([coinB.address], [2], 2));
            expect(await vault.paused()).to.equal(true);         // A's full pause still in effect
            expect(await vault.depositsFrozen()).to.equal(true); // B's deposit freeze added

            // Verify hold counts: one FULL_FREEZE hold, one DEPOSIT_ONLY_FREEZE hold
            expect(await holdLedger.activeHoldCount(vaultAddr)).to.equal(2n);
            expect(await holdLedger.fullFreezeHoldCount(vaultAddr)).to.equal(1n);

            // Drive coinA to recovery — releases hold A, partial downgrade fires
            for (let i = 0; i < STABILITY_WINDOW; i++) {
                await receiver.connect(forwarder).onReport(
                    "0x",
                    encodeReport([coinA.address], [0], 0)
            ...
            // After downgrade: unpause() fired (paused→false), pauseDeposits() fired (depositsFrozen stays true)
            expect(await vault.paused()).to.equal(false);
            expect(await vault.depositsFrozen()).to.equal(true);
            expect(await holdLedger.activeHoldCount(vaultAddr)).to.equal(1n);
            expect(await holdLedger.fullFreezeHoldCount(vaultAddr)).to.equal(0n);
        });
    });
